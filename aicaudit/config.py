"""Configuration loading for AICAudit.

Reads settings from:
1. pyproject.toml [tool.aicaudit] section (project level)
2. .aicauditignore file (path exclusions)
...
def load_pyproject(root: Path) -> dict:
    """Load [tool.aicaudit] from pyproject.toml. Never raises."""
    ...
    try:
        with open(pyproject, "rb") as f:
            data = tomllib.load(f)
        return data.get("tool", {}).get("aicaudit", {})
    except Exception:  # noqa: BLE001 - broken config should not crash the tool
        return {}


def load_ignore_file(root: Path) -> list[str]:
    """Read .aicauditignore (one glob per line, empty/comment lines skipped)."""
    path = root / ".aicauditignore"
    if not path.exists():
        return []
    patterns = []
...
def merge_config(cli_rules, cli_min_severity, start: Path | None = None) -> AuditConfig:
    """Merge config file + .aicauditignore + CLI overrides."""
    ...
    # ignore: config file patterns + .aicauditignore patterns
