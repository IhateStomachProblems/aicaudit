...
class PathTraversal(Rule):
    ...
    def check(self, tree, context):
        ...
        for node in ast.walk(tree):
            ...
            is_opener = func in FILE_OPENERS and node.args
            is_join = func in SUSPICIOUS_JOIN and node.args
            is_extract = func.endswith(("extract", "extractall")) and node.args
            if not (is_opener or is_join or is_extract):
                continue

            # Taint-aware mode (full scans): the engine proved which arguments
            # are constant/sanitized; only taint-or-unresolved arguments fire.
            if context.taint is not None:
                event = context.taint.event_for(node)
                if event is None:
                    continue
                if is_join or is_extract:
                    findings.append(self._make_join(node, func, context, event.paths))
                else:
            ...
            # Pattern 3: archive extraction (zip/tar slip)
            if is_extract:
    ...
    def _make(self, node, func, ctx, taint_path=None):
        return Finding(rule_id=self.id, message=f"Path traversal: user input in '{func}()'",
                       ...
                       cwe="CWE-22", taint_path=taint_path)

    def _make_join(self, node, func, ctx, taint_path=None):
